<nav class="relative container mx-auto p-5 font-mono font-bold">
    <div class="flex items-center justify-between ">
     <h2 class="pt-2 font-mono font-bold text-2xl"> Hygiene </h2>
     <!----- menu -->
     <div class="hidden md:flex space-x-6">
       <a href="/about" class="hover:text-violet-600">About</a>
       <a href="/about" class="hover:text-violet-600">About</a>
       <a href="/about" class="hover:text-violet-600">About</a>
       <a href="/about" class="hover:text-violet-600">About</a>
     </div>
     <div>
       <a href="/about"class="p-3  text-white rounded-full baseline bg-violet-500 hover:bg-violet-600 active:bg-violet-700 focus:outline-none focus:ring focus:ring-violet-300">Contact us</a>
   
     </div>
    </div>
    <div  id= "niv" class="lg:hidden mt-2 flex flex-row justify-between bg-teal-200">
      <a href="/about" class="p-2 bg-teal-200   hover:text-violet-600">About</a>
      <a href="/about" class="p-2 bg-teal-200  hover:text-violet-600">About</a>
      <a href="/about" class="p-2 bg-teal-200  hover:text-violet-600">About</a>
      <a href="/about" class="p-2 bg-teal-200  hover:text-violet-600">About</a>
      <a href="/about" class="p-2 bg-teal-200  hover:text-violet-600">About</a>
    </div>
   </nav>

<script>
  
</script>
